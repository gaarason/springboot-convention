...
/**
 * org.org.apache.tomcat.embed:tomcat-embed-core:9.0.43 org.org.apache.coyote.http11.Http11InputBuffer
 */
...
public class Http11InputBuffer implements InputBuffer, ApplicationBufferHandler {
    ...
    private static final Log LOG = LogFactory.getLog(org.apache.coyote.http11.Http11InputBuffer.class);

    /**
     * The string manager for this package.
     */
    private static final StringManager SM = StringManager.getManager(org.apache.coyote.http11.Http11InputBuffer.class);
    ...
    private static String parseInvalid(int startPos, ByteBuffer buffer) {
    ...
    void addFilter(InputFilter filter) {
        ...
        if (filter == null) {
            throw new NullPointerException(org.apache.coyote.http11.Http11InputBuffer.SM.getString("iib.filter.npe"));
        }
    ...
    boolean parseRequestLine(boolean keptAlive, int connectionTimeout, int keepAliveTimeout) throws IOException {
        ...
        if (parsingRequestLinePhase < 2) {
            do {
                ...
                if (!keptAlive && byteBuffer.position() == 0 && byteBuffer.limit() >= org.apache.coyote.http11.Http11InputBuffer.CLIENT_PREFACE_START.length - 1) {
                    boolean prefaceMatch = true;
                    for (int i = 0; i < org.apache.coyote.http11.Http11InputBuffer.CLIENT_PREFACE_START.length && prefaceMatch; i++) {
                        if (org.apache.coyote.http11.Http11InputBuffer.CLIENT_PREFACE_START[i] != byteBuffer.get(i)) {
                            prefaceMatch = false;
                        }
                    }
                    if (prefaceMatch) {
        ...
        if (parsingRequestLinePhase == 2) {
            ...
            while (!space) {
                ...
                if (chr == Constants.SP || chr == Constants.HT) {
                    ...
                    String invalidMethodValue = org.apache.coyote.http11.Http11InputBuffer.parseInvalid(parsingRequestLineStart, byteBuffer);
                    throw new IllegalArgumentException(org.apache.coyote.http11.Http11InputBuffer.SM.getString("iib.invalidmethod", invalidMethodValue));
                }
            }
        ...
        if (parsingRequestLinePhase == 4) {
            ...
            while (!space) {
                ...
                if (prevChr == Constants.CR && chr != Constants.LF) {
                    ...
                    // Avoid unknown protocol triggering an additional error
                    request.protocol().setString(Constants.HTTP_11);
                    String invalidRequestTarget = org.apache.coyote.http11.Http11InputBuffer.parseInvalid(parsingRequestLineStart, byteBuffer);

                    ////////////////////////
                    // 当tomcat出错时, 记录下当前的uri&query
                    ////////////////////////
                    WebMvcRequestUtils.writeTheRequestRealUri(invalidRequestTarget);

                    throw new IllegalArgumentException(
                        org.apache.coyote.http11.Http11InputBuffer.SM.getString("iib.invalidRequestTarget", invalidRequestTarget));
                }
                if (chr == Constants.SP || chr == Constants.HT) {
                    space = true;
                    end = pos;
        ...
        if (parsingRequestLinePhase == 6) {
            ...
            while (!parsingRequestLineEol) {
                ...
                if (chr == Constants.CR) {
                    ...
                    String invalidProtocol = org.apache.coyote.http11.Http11InputBuffer.parseInvalid(parsingRequestLineStart, byteBuffer);
                    throw new IllegalArgumentException(
                        org.apache.coyote.http11.Http11InputBuffer.SM.getString("iib.invalidHttpProtocol", invalidProtocol));
                }
            }
        ...
        throw new IllegalStateException(org.apache.coyote.http11.Http11InputBuffer.SM.getString("iib.invalidPhase", Integer.valueOf(parsingRequestLinePhase)));
    }
    ...
    boolean parseHeaders() throws IOException {
        if (!parsingHeader) {
            throw new IllegalStateException(org.apache.coyote.http11.Http11InputBuffer.SM.getString("iib.parseheaders.ise.error"));
        }
        ...
        do {
            ...
            if (byteBuffer.position() > headerBufferSize || byteBuffer.capacity() - byteBuffer.position() < socketReadBufferSize) {
                throw new IllegalArgumentException(org.apache.coyote.http11.Http11InputBuffer.SM.getString("iib.requestheadertoolarge.error"));
            }
    ...
    int available(boolean read) {
        ...
        try {
            ...
            if (org.apache.coyote.http11.Http11InputBuffer.LOG.isDebugEnabled()) {
                org.apache.coyote.http11.Http11InputBuffer.LOG.debug(org.apache.coyote.http11.Http11InputBuffer.SM.getString("iib.available.readFail"), ioe);
            }
    ...
    private boolean fill(boolean block) throws IOException {
        ...
        if (org.apache.coyote.http11.Http11InputBuffer.LOG.isDebugEnabled()) {
            org.apache.coyote.http11.Http11InputBuffer.LOG.debug(
                    "Before fill(): [" + parsingHeader + "], parsingRequestLine: [" + parsingRequestLine + "], parsingRequestLinePhase: [" + parsingRequestLinePhase
                            + "], parsingRequestLineStart: [" + parsingRequestLineStart + "], byteBuffer.position() [" + byteBuffer.position() + "]");
        }
        ...
        SocketWrapperBase<?> socketWrapper = wrapper;
        int nRead = -1;
        if (socketWrapper != null) {
            nRead = socketWrapper.read(block, byteBuffer);
        } else {
            throw new CloseNowException(org.apache.coyote.http11.Http11InputBuffer.SM.getString("iib.eof.error"));
        }
        byteBuffer.limit(byteBuffer.position()).reset();

        if (org.apache.coyote.http11.Http11InputBuffer.LOG.isDebugEnabled()) {
            org.apache.coyote.http11.Http11InputBuffer.LOG
                    .debug("Received [" + new String(byteBuffer.array(), byteBuffer.position(), byteBuffer.remaining(), StandardCharsets.ISO_8859_1) + "]");
        }

        if (nRead > 0) {
            return true;
        } else if (nRead == -1) {
            throw new EOFException(org.apache.coyote.http11.Http11InputBuffer.SM.getString("iib.eof.error"));
        } else {
            return false;
        }
    ...
    private HeaderParseStatus parseHeader() throws IOException {
        ...
        while (headerParsePos == HeaderParsePosition.HEADER_VALUE_START || headerParsePos == HeaderParsePosition.HEADER_VALUE
    ...
    private HeaderParseStatus skipLine() throws IOException {
        ...
        if (rejectIllegalHeader || org.apache.coyote.http11.Http11InputBuffer.LOG.isDebugEnabled()) {
            String message = org.apache.coyote.http11.Http11InputBuffer.SM.getString("iib.invalidheader",
                    HeaderUtil.toPrintableString(byteBuffer.array(), headerData.lineStart, headerData.lastSignificantChar - headerData.lineStart + 1));
            if (rejectIllegalHeader) {
                throw new IllegalArgumentException(message);
            }
            org.apache.coyote.http11.Http11InputBuffer.LOG.debug(message);
        }
